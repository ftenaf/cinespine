# Replit deployment uses this root Dockerfile
...
COPY .deployment/pyproject.toml ./pyproject.toml
COPY .deployment/uv.lock ./uv.lock

# Fix the 'where' path since pyproject.toml is now at the root of /app
RUN sed -i 's/where = \["\.\."\]/where = \["\."\]/' pyproject.toml
