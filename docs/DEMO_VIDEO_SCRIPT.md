...
3. **Warm the Wrap Rescue Agent.** Run it once for Day 31 off-camera. The MCP service scales to zero and its cold start is ~20 s; the second run is fast. Confirm the panel shows `mcp_available: true` and a Gemini memo. Then **open Day 31** in Set & Editorial Spine (or `curl` `/api/discrepancies?production_id=DEMO_PRODUCTION&shoot_day=31`): the Grafana "Active discrepancies now" tile is recorded when a day is opened, not scraped, and shows 3 about a minute later. Before `0cfc491` it read 0 no matter what; if it does again, that is a regression, not a cold cache.
